...
SimpleNavigation::Configuration.run do |navigation|
  navigation.items do |primary|
    ...
    primary.item "X-Post Settings (3rd Party)", safe_join([fa_icon('cog fw'), t('X-Post Settings (3rd party)')]), "https://crossposter.counter.social", link_html: { 'data-method' => 'delete' }
    ...
    primary.item :moderation, safe_join([fa_icon('gavel fw'), t('moderation.title')]), admin_reports_url, if: proc { current_user.staff? } do |admin|
      admin.item :action_logs, safe_join([fa_icon('bars fw'), t('admin.action_logs.title')]), admin_action_logs_url, if: -> { current_user.admin? }
